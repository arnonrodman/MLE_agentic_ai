...
def tavily_search_tool(
    ...
    # Disable SSL verification for TavilyClient if possible by patching requests
    # Or we can patch requests.post/get globally or for this module
    # Since TavilyClient uses requests, we can set session verify if it accepts it, but it creates its own session usually.
    # Let's patch requests.Session.request locally if needed or assume urllib3 disable works if Tavily uses a session.
    # Actually, TavilyClient creates a session internally.
    # We'll monkeypatch requests.Session.request to default verify=False
