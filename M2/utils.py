...
# Configure httpx client to handle SSL certificate issues (common in corporate environments)
# Set OPENAI_VERIFY_SSL=false in environment to disable SSL verification if needed
# WARNING: Disabling SSL verification is a security risk - only use in trusted environments
verify_ssl = os.getenv("OPENAI_VERIFY_SSL", "false").lower() != "false"  # Default to False to handle SSL issues
http_client = httpx.Client(verify=verify_ssl, timeout=60.0)

openai_client = OpenAI(
    api_key=openai_api_key,
    http_client=http_client
) if openai_api_key else OpenAI(http_client=http_client)
